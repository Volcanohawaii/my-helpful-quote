...
const suriPrescription = {
    getRemedy: (num, lang) => {
        ...
        const data = {
            ko: {
                actions: [
                    "과감한 실행", "내실 다지기", "창의적 발상", "감정 조절", "중재와 화합", "명예 수호", "독립적 결단", "인내와 끈기", "결실의 마무리",
                    "과감한 투자", "인맥 관리", "지식 습득", "환경 변화", "명상과 휴식", "체력 단련", "기록의 습관", "공간 정돈", "언어의 절제",
                    "겸손한 태도", "신속한 판단", "장기적 계획", "기술 연마", "예술적 몰입", "봉사와 나눔", "공정한 경쟁", "직관의 신뢰",
                    "정밀한 분석", "유연한 수용", "원칙의 준수", "새로운 도전"
                ],
                socials: [
                    "진취격(進取格)의 조력자", "정중격(靜重格)의 멘토", "심미격(審美格)의 영감자", "실리격(實利格)의 조언자", "덕망격(德望格)의 리더", 
                    "문창격(文昌格)의 학문가", "숙살격(肅殺格)의 전문가", "신의격(信義格)의 동반자", "원숙격(圓熟格)의 지혜자", "강건격(剛健格)의 수호자",
                    "유연격(柔軟格)의 협상가", "명철격(明哲格)의 가이드", "화합격(和合格)의 중재자", "예지격(叡智格)의 선구자", "풍요격(豊饒格)의 파트너",
                    "단단격(丹鍛格)의 장인", "광명격(光明格)의 활력가", "고결격(高潔格)의 동경자", "탐구격(探求格)의 동료", "결단격(決斷格)의 선배",
                    "포용격(包容格)의 후배", "통찰격(洞察格)의 감시자", "성실격(誠實格)의 관리자", "창조격(創造格)의 예술가", "안정격(安定格)의 후원자",
                    "정의격(正義格)의 투사", "자애격(慈愛格)의 치유자", "변혁격(變革格)의 혁신가", "신비격(神秘格)의 관찰자", "완성격(完成格)의 조율자"
                ],
                foods: [
                    "녹색 채소", "따뜻한 성질의 차", "붉은 과일", "단백질 위주", "뿌리 채소", "해조류", "견과류", "발효 식품", "블랙푸드(검은콩)",
                    "제철 나물", "맑은 생수", "비타민C 보충", "견과류 믹스", "따뜻한 우유", "신선한 샐러드", "잡곡밥", "구운 생선", "오메가3 식품",
                    "버섯 요리", "상큼한 과일", "매콤한 향신료", "부드러운 죽", "고소한 깨", "쌉싸름한 산나물", "달콤한 꿀", "시원한 오이",
                    "지방 적은 고기", "발효 식초", "따뜻한 호박", "신선한 토마토"
                ],
                colors: [
                    "그린(Green)", "네이비(Navy)", "레드(Red)", "옐로우(Yellow)", "화이트(White)", "골드(Gold)", "퍼플(Purple)", "브라운(Brown)", "실버(Silver)",
                    "스카이블루", "차콜그레이", "오렌지", "핑크", "민트", "베이지", "라벤더", "버건디", "인디고", "올리브", "터쿼이즈", "크림", "머스타드", 
                    "에메랄드", "로즈골드", "샌드베이지", "딥블루", "라임", "피치", "그레이", "블랙"
                ]
            },
            en: {
                actions: [
                    "Bold Execution", "Inner Stability", "Creative Thinking", "Emotional Control", "Mediation & Harmony", "Honor Protection", "Independent Decision", "Patience & Persistence", "Fruitful Conclusion",
                    "Bold Investment", "Networking", "Knowledge Gain", "Envir. Change", "Meditation", "Physical Training", "Record Keeping", "Organizing Space", "Mindful Speech",
                    "Humble Attitude", "Quick Judgment", "Long-term Plan", "Skill Refinement", "Artistic Flow", "Service & Sharing", "Fair Competition", "Trust Intuition",
                    "Precise Analysis", "Flexible Acceptance", "Following Rules", "New Challenge"
                ],
                socials: [
                    "Pioneer-rank Supporter", "Serene-rank Mentor", "Aesthetic-rank Inspirer", "Practical-rank Advisor", "Virtue-rank Leader", 
                    "Academic-rank Intellectual", "Precision-rank Specialist", "Faith-rank Companion", "Mature-rank Wise Elder", "Fortitude-rank Guardian",
                    "Flexible-rank Negotiator", "Brilliant-rank Guide", "Harmony-rank Mediator", "Visionary-rank Pioneer", "Abundance-rank Partner",
                    "Craft-rank Artisan", "Radiant-rank Energizer", "Noble-rank Admirer", "Research-rank Peer", "Decision-rank Senior",
                    "Tolerance-rank Junior", "Insight-rank Watcher", "Sincere-rank Manager", "Creative-rank Artist", "Stability-rank Sponsor",
                    "Justice-rank Fighter", "Benevolence-rank Healer", "Reform-rank Innovator", "Mystic-rank Observer", "Perfect-rank Harmonizer"
                ],
                foods: [
                    "Green Vegetables", "Warm Herbal Tea", "Red Fruits", "Protein-based", "Root Vegetables", "Seaweeds", "Assorted Nuts", "Fermented Foods", "Black Foods",
                    "Seasonal Herbs", "Pure Water", "Vitamin C", "Mixed Nuts", "Warm Milk", "Fresh Salad", "Multi-grains", "Grilled Fish", "Omega-3 Rich",
                    "Mushroom Dish", "Citrus Fruits", "Spicy Spices", "Soft Porridge", "Sesame Seeds", "Wild Herbs", "Sweet Honey", "Cool Cucumber",
                    "Lean Meat", "Natural Vinegar", "Warm Pumpkin", "Fresh Tomato"
                ],
                colors: [
                    "Green", "Navy", "Red", "Yellow", "White", "Gold", "Purple", "Brown", "Silver",
                    "Sky Blue", "Charcoal", "Orange", "Pink", "Mint", "Beige", "Lavender", "Burgundy", "Indigo", "Olive", "Turquoise", "Cream", "Mustard", 
                    "Emerald", "Rose Gold", "Sand Beige", "Deep Blue", "Lime", "Peach", "Gray", "Black"
                ]
            }
        };

        const d = data[lang];
        // [중요] 배열이 길어졌으므로 % 9 대신 % 배열길이 를 사용하여 모든 데이터를 활용합니다.
