    ...
    getRemedy: (num, lang) => {
        ...
        const data = {
            ko: { // <--- [수정] 중괄호 추가
                ...
                socials: ["진취격(進取格)의 조력자", "정중격(靜重格)의 멘토", "심미격(審美格)의 영감자", "실리격(實利格)의 조언자", "덕망격(德望格)의 리더", "문창격(文昌格)의 학문가", "숙살격(肅殺格)의 전문가", "신의격(信義格)의 동반자", "원숙격(圓熟格)의 지혜자", "강건격(剛健格)의 수호자", "유연격(柔軟格)의 협상가", "명철격(明哲格)의 가이드", "화합격(和合格)의 중재자", "예지격(叡智格)의 선구자", "풍요격(豊饒格)의 파너", "단단격(丹鍛格)의 장인", "광명격(光明格)의 활력가", "고결격(高潔格)의 동경자", "탐구격(探求格)의 동료", "결단격(決斷格)의 선배", "포용격(包容格)의 후배", "통찰격(洞察格)의 감시자", "성실격(誠實格)의 관리자", "창조격(創造格)의 예술가", "안정격(安定格)의 후원자", "정의격(正義格)의 투사", "자애격(慈愛格)의 치유자", "변혁격(變革格)의 혁신가", "신비격(神秘格)의 관찰자", "완성격(完成格)의 조율자"],
