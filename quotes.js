...
const suriPrescription = {
    getRemedy: (num, lackEls, lang) => {
        ...
        const data = {
            ko: {
                actions: [
    /* 목 */ "과감·실행(木)", "추진·강화(木)", "도전(木)", "아침 산책(木)", "식물 원예(木)", "목표 잡기(木)",
    /* 화 */ "창의·발상(火)", "활력·발산(火)", "소통·확대(火)", "일광욕(火)", "격한 운동(火)", "열띤 토론(火)",
    /* 토 */ "중재·화합(土)", "기반 축적(土)", "정리·정돈(土)", "러닝·걷기(土)", "명상·다도(土)", "느긋·휴식(土)",
    /* 금 */ "독립·결단(金)", "예리·분석(金)", "원칙·준수(金)", "근력 운동(金)", "루틴 잡기(金)", "호흡·훈련(金)",
    /* 수 */ "융통성(水)", "지혜 습득(水)", "심연·사색(水)", "수영·입욕(水)", "수분 섭취(水)", "낮잠·숙면(水)"
],
                socials: [
